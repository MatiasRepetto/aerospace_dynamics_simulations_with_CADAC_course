...
void merge_plot_files(string *plot_file_list,int num_cruise,const char *title);
...
void parse_plot_traj_csv(string *plot_files, int num_cruise, bool merge, string type);
...
int main(void) 
{
	...
	{
		//Loading pointers of the vehicle-object classes into 
	...
	if(strstr(options,"y_csv"))
	{
		if(strstr(options,"y_plot"))
		{
			...
			{
				parse_plot_traj_csv(plot_file_list,num_cruise,1,"plot");
			}
			else{
				parse_plot_traj_csv(plot_file_list,num_cruise,0,"plot");
			}
		}
