...
//Aerodynamic module
//Member function of class 'Cruise'
// Generates drag polar of cruise missile 'Cruise3'
...
//(3) Provides the c.g. location as a function of missile mass
//
					   ...
					   ,int num_satellite)
{
	...
	for(int k=0;k<num_satellite;k++)
	{
		if(!satellite_found)
		{
			...
			if(active)
			{
				...
				//determining closet target
				range=BIG;

				//j target counter; i vehicle counter, k satellite counter
				int j=0;
	...
	if(satellite_found)
	{
		...
		// (currently for 'guidance' module only 'wp_lonx', 'wp_latx', and 'wp_alt' are needed)
...
///////////////////////////////////////////////////////////////////////////////  
//Determining whether satellites are visible from 'this' missile and can track targets
// Checking for:	(1) are 'this' missile and satellites in line-of-sight
...
void Cruise::targeting_satellite(Packet *combus,int num_vehicles)
{
	...
	for(i=0;i<num_vehicles;i++)
	{
		...
		{
			id=combus[i].get_id();
			int loc=id.find("t"); //Schildt p.687, returns index (0, first letter) of match
			if(!loc)
			{
				first_target=true;
	...
	{
		id=combus[i].get_id();
		int loc=id.find("s"); //Schildt p.687, returns index (0, first letter) of match
		...
		//satellite is found
		{
			...
			//Determining clear line-of-sight between satellite and missile
			...
			//visible if altitude of missile is sufficient
			{
				//critical altitude
				double radius_crit;
...
///////////////////////////////////////////////////////////////////////////////  
//Calculating ground distances of missile to all targets
...
void Cruise::def_seeker()
{
	...
	cruise[112].init("targ_com_slot","int",0,"Slot # of target,attacked by missile, in 'combus' - ND","seeker","save","");
...
//			= 3 seeker tracking, sets mguidance=66, mcontrol=44
//			= 4 seeker break-lock (not implemented)
//
//NOTE:
...
void Cruise::seeker(Packet *combus,int vehicle_slot,int num_vehicles,int num_target)
{
	...
	Matrix STBI(3,1);  //displacement of target wrt missile in inertial coordinates
	double inv_dtb(0); //inverse of range-to-go
	Matrix VTBG(3,1);  //velocity of target wrt missle
	Matrix VBTG(3,1);  //velocity of missile wrt target
	Matrix DUM33(3,3);
	Matrix DUM3(3,1);
	...
	if(!acquisition&&(mseeker==1))
	{
		for(int j=0;j<num_target;j++)
		{
			...
			if(range<acq_range)
			{
				...
				//getting missile # (current vehicle = current'combus' slot) and target #
	...
	if(mseeker==3)
	{
		...
		//calculating relative missile-target displacement in missile geo. coord
		STBI=STII-SBII;
		TGI=TIG.trans();
		STBG=TGI*STBI;
...
Matrix Cruise::guidance_line()
{
	...
	//waypoint wrt missile displacement in geographic coord (synthetic LOS)
	...
	//ground range to waypoint (approximated by using missile local-level plane)
	swbg1=SWBG.get_loc(0,0);
	swbg2=SWBG.get_loc(1,0);
	wp_grdrange=sqrt(swbg1*swbg1+swbg2*swbg2);

	//converting geographic missile velocity to LOS and LOA coordinates
	VBEO=TOG*VBEG;
...
double Cruise::guidance_arc()
{
	...
	//displacement of waypoint wrt missile in geographic coord
	SWBG=TIG.trans()*(SWII-SBII);
	...
	//horizontal ground distance based on missile geographic coordinates
